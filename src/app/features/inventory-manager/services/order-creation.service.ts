...
export class OrderCreationService {
  ...
  submitOrderRequest(payload: Record<string, unknown>, isEditMode: boolean, orderId?: string): Observable<PurchaseRequest> {
    if (isEditMode && orderId) {
      return this.apiService.patch<PurchaseRequest>(`/inventory/order-requests/${orderId}`, payload);
    }
    return this.apiService.post<PurchaseRequest>('/inventory/order-requests', payload);
  }
