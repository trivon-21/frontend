...
export class InvoiceAutoCancelledComponent implements OnInit {
  ...
  applyFilters(): void {
    this.filteredInvoices = this.invoices.filter(i => {
      ...
      const md = this.selectedDate ? new Date(i.cancelledAt || i.updatedAt).toDateString() === new Date(this.selectedDate).toDateString() : true;
      return ms && md;
    });
  ...
  loadRepairInvoices(): void {
    ...
    this.invoiceService.getRepairAutoCancelledInvoices().subscribe({
  ...
  applyRepairFilters(): void {
    this.filteredRepairInvoices = this.repairInvoices.filter(i => {
      ...
      const md = this.repairSelectedDate ? new Date(i.cancelledAt || i.updatedAt).toDateString() === new Date(this.repairSelectedDate).toDateString() : true;
      return ms && md;
    });
