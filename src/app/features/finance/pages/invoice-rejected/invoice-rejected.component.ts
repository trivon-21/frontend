...
@Component({
  ...
  styleUrls: ['./invoice-rejected.component.css']
})
export class InvoiceRejectedComponent implements OnInit {
  ...
  loadInvoices(): void {
    this.isLoading = true;
    this.invoiceService.getRejectedInvoices().subscribe({
  ...
  applyFilters(): void {
    this.filteredInvoices = this.invoices.filter(i => {
      ...
      const md = this.selectedDate ? new Date(i.rejectedAt || i.updatedAt).toDateString() === new Date(this.selectedDate).toDateString() : true;
      return ms && md;
    });
  ...
  loadRepairInvoices(): void {
    ...
    this.invoiceService.getRepairRejectedInvoices().subscribe({
  ...
  applyRepairFilters(): void {
    this.filteredRepairInvoices = this.repairInvoices.filter(i => {
      ...
      const md = this.repairSelectedDate ? new Date(i.rejectedAt || i.updatedAt).toDateString() === new Date(this.repairSelectedDate).toDateString() : true;
      return ms && md;
    });
