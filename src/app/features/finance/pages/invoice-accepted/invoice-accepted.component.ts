...
@Component({
  selector: 'app-invoice-accepted',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './invoice-accepted.component.html',
  styleUrls: ['./invoice-accepted.component.css']
})
export class InvoiceAcceptedComponent implements OnInit {
  ...
  applyFilters(): void {
    this.filteredInvoices = this.invoices.filter(i => {
      ...
      const md = this.selectedDate ? new Date(i.acceptedAt || i.updatedAt).toDateString() === new Date(this.selectedDate).toDateString() : true;
      return ms && md;
    });
  ...
  applyRepairFilters(): void {
    this.filteredRepairInvoices = this.repairInvoices.filter(i => {
      ...
      const md = this.repairSelectedDate ? new Date(i.acceptedAt || i.updatedAt).toDateString() === new Date(this.repairSelectedDate).toDateString() : true;
      return ms && md;
    });
